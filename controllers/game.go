...
func (c *GameController) GetAll() {
	...
	// contains: game.genres
	if v := c.GetString("priceRange"); v != "" {
		priceRange = v
	}
	// count: true,false
	if v := c.GetString("count"); v != "" {
	...
	l, err := models.GetAllGame(query, fields, sortby, order, offset, limit, priceRange, count)
	if err != nil {
		c.Data["json"] = err.Error()
	} else {
		c.Data["json"] = l
